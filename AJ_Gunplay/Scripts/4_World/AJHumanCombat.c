modded class DayZPlayerImplement
{
	...
	override bool IsShootingFromCamera()
	{
		...
		if (player)
		{
			...
			if (AJIsInAimingView(player, weapon))
			{
				return super.IsShootingFromCamera();
			}
			...
			if (adminConfig && adminConfig.CrosshairMode == 2 && !AJHasHipfireLaser(weapon))
			{
				return super.IsShootingFromCamera();
			}
		}
	...
	protected bool AJIsInAimingView(PlayerBase player, Weapon_Base weapon)
	{
	...
	protected bool AJHasHipfireLaser(Weapon_Base weapon)
	{
		if (!weapon)
		{
			return false;
		...
		if (weapon.GetInventory())
		{
			for (int i = 0; i < weapon.GetInventory().AttachmentCount(); i++)
			{
				...
				if (attachment && AJIsLaserType(attachment.GetType()))
				{
					return true;
				}
			}
	...
	protected bool AJIsLaserType(string itemType)
	{
