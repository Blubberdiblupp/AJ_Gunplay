class AJFirearmsMechanics
{
	...
	static vector CalculateHipFireSpread(Weapon_Base weapon, PlayerBase player, vector aimDirection)
	{
		...
		float spreadAngle = config.HipFireAccuracyDegradation * weapon.GetAJHipFireSpreadModifier();
		...
		if (config.DebugMode)
		{
			Print("[AJ Gunplay] Hip Fire Spread: " + spreadVector + " (Original: " + aimDirection + ")");
		}
	...
	static vector ApplyHipFireCorrection(Weapon_Base weapon, PlayerBase player, vector originalDirection)
	{
		...
		if (AJVehicleCombatHandler.IsPlayerInVehicle(player))
		{
			if (config.DebugMode)
			{
				string vehicleType = AJVehicleCombatHandler.GetVehicleType(player);
				Print("[AJ Gunplay] Player in " + vehicleType + " - Hip-Fire Nerf SKIPPED");
			}
			return originalDirection;
		}
		...
		if (config.DebugMode)
		{
			Print("[AJ Gunplay] Hip Fire Correction Applied");
